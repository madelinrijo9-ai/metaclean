...
import { isTauri, saveFileNative, showNotification, saveCleanedSongNative, openInFinderNative } from "@/lib/tauri-bridge";
...
export function useMetaClean() {
  ...
  const cleanAll = useCallback(async () => {
    ...
    // Batch completion native notification
    showNotification(
      "Cleaning Complete",
      `Successfully processed ${toClean.length} audio file(s).`
  ...
  const downloadFile = useCallback(async (id: string) => {
    ...
    // Tauri Native direct save to Downloads/MetaClean/[Artist] - [Album]/
    if (isTauri()) {
      try {
        const artist = file.customMetadata?.artist || file.metadata?.artist || "";
        const album = file.customMetadata?.album || file.metadata?.album || "";
        const savedPath = await saveCleanedSongNative(outputName, artist, album, file.cleanedBlob);
        if (savedPath) {
          console.log(`Saved file natively: ${savedPath}`);
          toast.success(`Saved successfully to Downloads/MetaClean folder!`, {
            description: outputName,
            duration: 8000,
            action: {
              label: "Open in Finder",
              onClick: () => {
                openInFinderNative(savedPath).catch((e) => console.error(e));
              },
            },
          });
          showNotification("Song Saved", `Saved to Downloads/MetaClean folder: ${outputName}`).catch((e) => console.error(e));
        }
      } catch (err) {
    ...
    if (typeof window !== "undefined" && "showSaveFilePicker" in window) {
      try {
        const fileExt = file.cleanedExt || getExt(file.file.name);
        const mimeType = file.cleanedBlob.type || `audio/${fileExt === "mp3" ? "mpeg" : fileExt}`;
        // @ts-ignore
        ...
        await writable.write(file.cleanedBlob);
        ...
        if (err && err.name === "AbortError") {
          // User closed/cancelled the picker, don't download
          return;
        }
        console.warn("showSaveFilePicker failed or was rejected, falling back to standard download link", err);
      }
    }

    // Classic Web Fallback (automatic download to browser's Downloads folder)
  ...
  const downloadAll = useCallback(async () => {
    ...
    if (doneFiles.length === 1) {
      await downloadFile(doneFiles[0].id);
      return;
    }

    // Tauri Native direct save for all files
    if (isTauri()) {
      let savedCount = 0;
      let lastSavedPath = "";
      ...
      if (savedCount > 0) {
        toast.success(`Successfully saved ${savedCount} file(s) to Downloads/MetaClean folder!`, {
          duration: 8000,
          action: {
            label: "Open in Finder",
            onClick: () => {
              openInFinderNative(lastSavedPath).catch((e) => console.error(e));
            },
          },
        });
        showNotification("All Files Saved", `Saved ${savedCount} file(s) natively inside Downloads/MetaClean folder.`).catch(e => console.error(e));
      }
      return;
    }
    ...
    if (typeof window !== "undefined" && "showSaveFilePicker" in window) {
      try {
        // @ts-ignore
        const handle = await window.showSaveFilePicker({
          suggestedName: "metaclean-audio.zip",
          types: [
            {
